...
Worked manifest-backed abilities are available at [`examples/skills/solar-flare.skill.json`](examples/skills/solar-flare.skill.json) and [`examples/skills/prism-burst.ability.json`](examples/skills/prism-burst.ability.json). Import or upload either through the right-side VFX Editor, choose **Apply manifest to VFX editor**, then save the routing to test it in the VFX Lab and Hero Studio. **Upload + validate skill…** sends `.skill.json` or `.ability.json` through the asset API, rejects malformed manifests before saving, and rehydrates valid uploads on the next Hero Studio load; valid but unregistered IDs remain review-only. The importer boundaries and add/remove workflow are documented in [`examples/skills/README.md`](examples/skills/README.md) and [`skill.md`](skill.md).
