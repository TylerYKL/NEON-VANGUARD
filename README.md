...
A worked future-skill manifest is available at [`examples/skills/solar-flare.skill.json`](examples/skills/solar-flare.skill.json). It is intentionally specification-only: current JSON import accepts VFX settings presets or `hero_tuning.json`, not new gameplay ability registration. The importer boundaries and required code-registration steps are documented in [`examples/skills/README.md`](examples/skills/README.md).
