...
Last verified: 2026-09-11 (**Hero Studio (Beta)**; all hero models/skills routed through the combined Linear VFX surface; legacy workflow embedded)
...
| `hero-studio.html` | **Hero Studio (Beta)** — five approved casts plus the Solar Flare test ability, playable Aegis/Nyx/Lyra Q/E/R routing, model/skill/audio assignment, profiles, GLB import, PNG gallery, and expandable legacy workflow |
...
├── character-bay.html      DELIVERABLE — character viewer + live uploaded GLB library
...
├── build.mjs               esbuild → inline → three deliverables
├── shell/                  HTML shells + all CSS/HUD (game.html, bay.html, studio.html) — build inputs
...
└── src/
    ...
    ├── showcase.js   character bay (procedural roster + live uploaded GLB library)
    ├── gltfutil.js    DOM-free GLB parse/stats/normalise + the stage stamp a clone keeps (bay/studio/herofit)
...
`shell/bay.html`), not in a built file. The three root `*.html` deliverables are generated by
`node build.mjs` — never hand-edit them.
